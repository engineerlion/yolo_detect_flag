...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    xc = prediction[..., 4] > conf_thres  # candidates #prediction[...,4]这一位的值表示框中有物体的概率，这个概率和后面的分类概率相乘才得到每一类的预测概率
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        #选出conf大于thres的第xi张图片的所有预测bbox信息
        ...
        # Batched NMS
        #x[0,1,2,3]:bbox,x[4]:confidence,x[5]:class
...
def apply_classifier_ljh(x, model, img, im0):
    # Apply a second stage classifier to yolo outputs
    im0 = [im0] if isinstance(im0, np.ndarray) else im0
    #import pdb;pdb.set_trace()
    for i, d in enumerate(x):  # per image
